...
unsigned int hashtab_hash(char *key) {
    ...
    return h % HASH_SIZE;
        // elf
    // unsigned int h = 0, hash_mul = 0;
    // while (*key)
    // {
    //     h = (h << 4) + (*key++);//1
    //     if ((hash_mul == h & 0xf0000000) != 0) 
    //     {
    //          h ^= (hash_mul >> 24); // Затрагивается 5-8 бит, смешивается один раз 3
    //          h &= ~ hash_mul; // Очистить четыре старших бита 4
    //     }
    //     key++;
    // }   
    // return h % HASH_SIZE;
