...
bstree *
bstree_create (char *key, int value)
{
...
bstree *
bstree_lookup (bstree *tree, char *key)
{
  while (1)
    {
      if (!strcmp (tree->key, key))
...
static void
_bstree_delete1 (bstree *deltree, char *key)
{
...
static void
_bstree_delete2 (bstree *deltree, char *key)
{
  /* one child (left) */
  bstree *pardeltree = deltree->parent;
...
static void
_bstree_delete3 (bstree *deltree, char *key)
{
  char *res_key;
  ...
  strcpy (deltree->key, res_key);
}

bstree *
bstree_delete (bstree *tree, char *key)
{
...
void
print_tree_as_list (bstree *root)
{
  if (!root)
    return;
  if (root)
...
void
print_tree (bstree *tree, int p, int s)
{ // string s = string("")) {
  if (tree == NULL)
    return;
...
int
bstree_heigth (const bstree *p)
{
  int l, r, h = 0;
  if (p != NULL)
    {
      l = bstree_heigth (p->left);
      r = bstree_heigth (p->right);
      h = ((l > r) ? l : r) + 1;
    }
  return h;
}