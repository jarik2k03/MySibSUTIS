...
{
    for (int ii = 0; ii < N; ii += BS) {for (int jj = 0; jj < N; jj += BS) {for (int kk = 0; kk < N; kk += BS) {for (int i = ii; i < IMIN(N, ii + BS); i++) {for (int j = jj; j < IMIN(N, jj + BS); j++) {for (int k = kk; k < IMIN(N, kk + BS); k++)c[i][j] +=a[i][k] *   b[k][j];}}}}}
}
